function showRates() { // showrates function
    ...
    let request = new XMLHttpRequest(); //activate the HHTP request
    request.open('GET', 'https://api.freecurrencyapi.com/v1/latest?apikey=fca_live_oq1A4Y9nMCPWlD7qVCsPDVIwpbrmsb9b57M5Qdvp&currencies=USD%2CCAD%2CSEK%2CNZD%2CAUD&base_currency=EUR', true); // HTTP Request methdo is 'GET'. The URL endpoint specifies the API api parameters like the key,currencies and currency
    
    request.onload = function() { //Defines the next step when the XMLHttpRequesT load response is activated

        if (request.status == 200) { // checks if the response is successful
    ...
    request.send(); // Sends the XMLHttpRequest to the currency conversion API
}
